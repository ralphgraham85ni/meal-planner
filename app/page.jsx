...
// ─── Seed data ────────────────────────────────────────────────────────────────
const SEED_MEALS = [
  { id:"b1", name:"Porridge with banana",       type:"breakfast", cost:0.65, baby:true,  url:null, portions:5 },
  { id:"b2", name:"Scrambled eggs on toast",    type:"breakfast", cost:0.90, baby:true,  url:null, portions:5 },
  { id:"b3", name:"Weetabix & milk",            type:"breakfast", cost:0.45, baby:true,  url:null, portions:5 },
  { id:"b4", name:"Yoghurt & fruit",            type:"breakfast", cost:0.75, baby:true,  url:null, portions:5 },
  { id:"b5", name:"Pancakes with fruit",        type:"breakfast", cost:1.10, baby:false, url:null, portions:4 },
  { id:"b6", name:"Beans on toast",             type:"breakfast", cost:0.70, baby:false, url:null, portions:4 },
  { id:"b7", name:"Overnight oats",             type:"breakfast", cost:0.60, baby:true,  url:null, portions:5 },
  { id:"l1", name:"Homemade veg soup & bread",  type:"lunch",     cost:1.20, baby:true,  url:null, portions:5 },
  { id:"l2", name:"Cheese toasties & tomato",   type:"lunch",     cost:1.10, baby:false, url:null, portions:4 },
  { id:"l3", name:"Jacket potato with beans",   type:"lunch",     cost:0.95, baby:true,  url:null, portions:5 },
  { id:"l4", name:"Egg salad sandwiches",       type:"lunch",     cost:1.05, baby:false, url:null, portions:4 },
  { id:"l5", name:"Pasta & pesto",              type:"lunch",     cost:0.85, baby:true,  url:null, portions:5 },
  { id:"l6", name:"Lentil & veg puree",         type:"lunch",     cost:0.70, baby:true,  url:null, portions:3 },
  { id:"l7", name:"Fish fingers & peas",        type:"lunch",     cost:1.40, baby:false, url:null, portions:4 },
  { id:"d1", name:"Spaghetti bolognese",        type:"dinner",    cost:2.20, baby:true,  url:null, portions:5 },
  { id:"d2", name:"Slow cooker chicken stew",   type:"dinner",    cost:2.50, baby:true,  url:null, portions:5 },
  { id:"d3", name:"Fish pie",                   type:"dinner",    cost:2.80, baby:true,  url:null, portions:5 },
  { id:"d4", name:"Veggie chilli & rice",       type:"dinner",    cost:1.80, baby:true,  url:null, portions:5 },
  { id:"d5", name:"Chicken & veg tray bake",    type:"dinner",    cost:2.60, baby:true,  url:null, portions:5 },
  { id:"d6", name:"Lentil dhal & naan",         type:"dinner",    cost:1.60, baby:true,  url:null, portions:5 },
  { id:"d7", name:"Homemade burgers & wedges",  type:"dinner",    cost:2.90, baby:false, url:null, portions:4 },
  { id:"d8", name:"Macaroni cheese",            type:"dinner",    cost:1.70, baby:true,  url:null, portions:5 },
  { id:"d9", name:"Sausage & mash",             type:"dinner",    cost:2.10, baby:true,  url:null, portions:5 },
  { id:"d10",name:"Beef & veg soup",            type:"dinner",    cost:2.00, baby:true,  url:null, portions:5 },
];
...
export default function App() {
  ...
  const [newMeal, setNewMeal]   = useState({ name:"", type:"dinner", cost:"", baby:false, url:"", portions:5 });
  ...
  async function saveMeal() {
    ...
    const mealRow = {
      ...
      portions: parseInt(newMeal.portions) || 5,
    };
    ...
    await loadAll();
    setNewMeal({ name:"", type:"dinner", cost:"", baby:false, url:"", portions:5 });
  ...
  const totalCost = DAYS.reduce((acc, d) =>
    acc + MEAL_TYPES.reduce((s, t) => {
      const slot = plan[d]?.[t] || {};
      const m1 = mealById[slot.m1];
      const m2 = slot.m2 && slot.m2 !== "" ? mealById[slot.m2] : null;
      return s
        + (m1 ? m1.cost * (m1.portions || 5) : 0)
        + (m2 ? m2.cost * (m2.portions || 5) : 0);
    }, 0)
  , 0);
    ...
    <div className="app">
      ...
      <header className="hdr">
        <div>
          ...
          <p>Shared family planner — portions set per meal</p>
        </div>
          ...
          <div className="add-form">
            ...
            <div className="frow">
              ...
              <input className="finput" placeholder="Cost per person £" type="number" step="0.01"
                style={{maxWidth:140}} value={newMeal.cost}
                onChange={e => setNewMeal(m => ({ ...m, cost: e.target.value }))} />
              <input className="finput" placeholder="Portions (people)" type="number" min="1" max="20"
                style={{maxWidth:150}} value={newMeal.portions}
                onChange={e => setNewMeal(m => ({ ...m, portions: e.target.value }))} />
                ...
                <div className="igrid">
                  {typeMeals.map(m => (
                    <div key={m.id} className="meal-card-mini">
                      ...
                      <div className="mm">
                        <span style={{fontWeight:700,color:"#e8a838"}}>£{(m.cost*(m.portions||5)).toFixed(2)} ({m.portions||5} 👤)</span>
              ...
              <div key={m.id} className="mopt" onClick={() => pickMeal(m.id)}>
                <div>
                  ...
                  <div className="mm">
                    ...
                    {(ingMap[m.id]||[]).length} ingredients · {m.portions||5} people
                  </div>
                </div>
                <div className="mp">£{(m.cost*(m.portions||5)).toFixed(2)}</div>
              </div>
            ))}
            <div className="btns">
        ...
        return (
          <div className="overlay" onClick={() => setModal(null)}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="sub">
                {m?.baby ? "✅ Baby-friendly" : "👶 Adapt for baby"}
                {" · "}£{((m?.cost||0)*(m?.portions||5)).toFixed(2)} for {m?.portions||5} people
              </div>
              {m?.url && (
